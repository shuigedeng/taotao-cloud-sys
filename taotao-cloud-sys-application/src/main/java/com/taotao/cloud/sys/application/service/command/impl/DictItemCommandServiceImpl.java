...
import com.taotao.cloud.sys.application.service.command.DictItemCommandService;
 ...
 */
@Service
public class DictItemCommandServiceImpl implements DictItemCommandService {
    ...
    // predicate.and(DICT_ITEM.description.like(description)));
    ...
    //        BooleanExpression predicate = DICT_ITEM.delFlag.eq(false);
    //        Optional.ofNullable(dictItemQuery.getDictId()).ifPresent(dictId ->
    // predicate.and(DICT_ITEM.dictId.eq(dictId)));
    ...
    //                .ifPresent(description ->
    // predicate.and(DICT_ITEM.description.like(description)));
    //        Optional.ofNullable(dictItemQuery.getStatus()).ifPresent(status ->
    // predicate.and(DICT_ITEM.status.eq(status)));
