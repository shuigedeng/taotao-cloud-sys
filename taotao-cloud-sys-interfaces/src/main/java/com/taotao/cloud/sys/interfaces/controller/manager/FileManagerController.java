...
public class FileManagerController extends BusinessController {
    ...
    //    public Result<Boolean> qqqqqqBatch(@Valid @NotEmpty(message = "数据不能为空") @RequestBody
    // ValidList<Student> ids) {
    //        return Result.success(true);
    //    }
    //
    //    @NotAuth
    ...
    //            responses = {@ApiResponse(description = "是否成功", content = @Content(mediaType =
    // "application/json"))})
    //    @Parameters({
    //            @Parameter(name = "type", required = true, description = "类型", schema =
    // @Schema(type = "integer")),
    //            @Parameter(name = "file", required = true, description = "文件信息", schema =
    // @Schema(type = "file"))
    //    })
    //    @PostMapping(value = "/upload")
    //    public Result<UploadFileVO> upload(@NotNull(message = "类型不能为空") @RequestParam("type")
    // Integer type,
    //                                       @NotNull(message = "文件不能为空") @RequestPart("file")
    // MultipartFile file) {
    ...
    // "userId") Long userId,
    //                                           @Validated @NotEmpty(message = "角色id列表不能为空")
    ...
    //                                @RequestParam("birthDay") @JsonFormat(pattern = "yyyy-MM-dd
    // HH:mm:ss") @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") LocalDateTime birthDay,
    //                                @RequestParam("plays") String[] plays,
    //                                @RequestParam("plays2") List<String> plays2) {
    //        LogUtils.info("请求参数： name = {}, age = {}, birthDay = {}, plays = {}, plays2 = {}",
    // name, age, birthDay, plays, plays2);
    ...
    //            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(content =
    // @Content(mediaType = "application/json")),
    //            responses = {@ApiResponse(description = "helloParam", content = @Content(mediaType
    // = "application/json"))})
    //    @GetMapping(value = "/helloParam3")
    //    public Result<String> hello(@Validated Page page) {
    //        LogUtils.info("请求参数： name = {}, age = {}, birthDay = {}, 分页参数：page = {}", "", "", "",
    // page);
    //        return Result.success("scesccc");
    //    }
    //
    ...
    ////            @Parameter(name = "birthDay", required = true, description = "birthDay", schema
    ...
    //            responses = {@ApiResponse(description = "application", responseCode = "200",
    ...
    ////    @NotAuth
    ////    @GetMapping(value = "/sku/page")
    ////    public Result<PageResult<Integer>> getSkuByPage(@Validated GoodsPageQuery
    // goodsPageQuery) {
    ////        return Result.success(new PageResult<Integer>());
    ////    }
    //
    ...
    //        @Schema(name = "pageSize", type = "integer", description = "当前页", requiredMode =
    // Schema.RequiredMode.REQUIRED)
    //        private Integer pageSize;
    //
    //        @Schema(name = "size", type = "integer", description = "每页数量", requiredMode =
    // Schema.RequiredMode.REQUIRED)
    ...
    //    // public ResultMessage<IPage<File>> getFileList(@RequestHeader String accessToken, File
    // file,
    ...
    //    //	return ResultUtil.data(fileService.customerPageOwner(fileOwnerDTO, file, searchVO,
    // pageVo));
    //    // }
    //    //
	...
	////    public Result<List<UploadFileVO>> uploadMultipleFiles(@RequestPart("files")
	// MultipartFile[] files) {
	////        if (files.length == 0) {
	...
	//    // @ApiImplicitParams({
	//    // 	@ApiImplicitParam(name = "token", value = "登录授权码", required = true, paramType =
	// "header",
	//    // dataType = "String"),
	//    // 	@ApiImplicitParam(name = "fileName", value = "路径名称", required = true, dataType =
	// "String",
