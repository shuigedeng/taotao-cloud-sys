...
public class UserManagerController extends BusinessController {
    ...
    //	@Parameters({
    //		@Parameter(name = "phone", description = "手机号码", required = true, example = "15730445331",
    // in = ParameterIn.PATH)
    //	})
    //	@RequestLogger
    ...
    //		@NotNull(message = "用户id不能为空") @PathVariable(name = "userId") Long userId,
    //		@Validated @NotEmpty(message = "角色id列表不能为空") @RequestBody RestPasswordUserDTO
    // restPasswordDTO) {
    ...
    //	}
     ...
     "后台页面-用户信息页面-根据用户id更新角色信息(用户分配角色)")
    	...
    	public Result<Void> assignUserRoles( @Valid @RequestBody AssignRolesCommand assignUserRolesCommand) {
    ...
    //		responses = {
    //			@ApiResponse(description = "更新角色信息是否成功", responseCode = "200", content = @Content(mediaType
