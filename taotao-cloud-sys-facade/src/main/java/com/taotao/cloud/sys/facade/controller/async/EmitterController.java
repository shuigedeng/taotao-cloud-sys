package com.taotao.cloud.sys.facade.controller.async;
	...
	public SseEmitter sseEmitter(@PathVariable String userId) {
		...
		CompletableFuture.runAsync(() -> {
			try {
				// 模拟耗时操作并推送数据
				for (int i = 0; i < 10; i++) {
					SseEmitter.SseEventBuilder event = SseEmitter.event().data("sseEmitter " + userId + " @ " + LocalTime.now()).id(String.valueOf(userId)).name("sseEmitter");
					emitter.send(event);
					Thread.sleep(2000);
				}
				// 完成
